...
class Heading2Rule(Rule):
    type='heading2'
    def condition(self, block):
        return block[0:2]=='##'
    def action(self, block, handler):
        handler.start(self.type)
        handler.feed(block[2:])
        handler.end(self.type)
        return True
    
class Heading3Rule(Rule):
    type='heading3'
    def condition(self, block):
        return block[0:3]=='###'
    def action(self, block, handler):
        handler.start(self.type)
        handler.feed(block[3:])
        handler.end(self.type)
        return True

class Heading4Rule(Rule):
    type='heading3'
    def condition(self, block):
        return block[0:4]=='####'
    def action(self, block, handler):
        handler.start(self.type)
        handler.feed(block[4:])
        handler.end(self.type)
        return True

class Heading5Rule(Rule):
    type='heading3'
    def condition(self, block):
        return block[0:5]=='#####'
    def action(self, block, handler):
        handler.start(self.type)
        handler.feed(block[5:])
        handler.end(self.type)
        return True
